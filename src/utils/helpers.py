...
def rgb_loss(params: th.Tensor, target: th.Tensor) -> th.Tensor:
    # 将输入参数分成两个部分
    pr, alpha, beta = th.chunk(params, 3, dim=1)  # 修改为3个参数

    # 将目标转换为范围[0, 1]
    # 灰度图的目标应该是一个通道
    target = (2 * target - 1)  # 归一化到[-1, 1]
    
    # 将参数通过sigmoid函数进行非线性变换
    pr = binquant(th.sigmoid(pr))
    
    # 计算损失
    loss = -th.sum(th.log2(pr) * target + th.log2(1 - pr) * (1 - target), dim=(0, 2, 3))
...
def get_p1(params: th.Tensor, target: th.Tensor) -> th.Tensor:
    # 将参数划分为 pr、alpha 和 beta
    pr, alpha, beta = th.chunk(params, 3, dim=1)
    
    # 灰度图目标归一化到 [-1, 1]
    r = 2 * target - 1
    
    # 将 `pr` 和 `pr + r * alpha + r * beta` 进行 sigmoid 变换
    pr = th.sigmoid(pr + r * alpha + r * beta)
...
def get_i1(params: th.Tensor, target: th.Tensor) -> th.Tensor:
    # 对 `get_p1_gray` 计算结果进行四舍五入，并将结果限制在 [1, bin_scale - 1] 范围
    return th.round(get_p1(params, target)).clamp(1, bin_scale - 1).to(th.uint8)
...
def decode_with_params(params: th.Tensor, coder: MixDecoder) -> th.Tensor:
    # 仅处理灰度图的参数
    pr, alpha , _= th.chunk(params, 3, dim=1)  # 灰度图仅需两个参数
    ...
    # 返回单通道的灰度图
    return r.unsqueeze(1)  # 保持 (N, C, H, W) 的格式，C=1

...
class SIG(nn.Module):
    def __init__(self,
        ...
        self.plane_context = UNetModel(1,
                                     plane_width,
                                     plane_depth,
                                     (),
                                     dropout,
    ...
    def forward(self, x:th.Tensor, qps:list=None) -> th.Tensor:
        ...
        hyperprior = hyperprior.repeat(1, 1, 1, 1)
        ...
        loss_px = th.zeros(8*self.sig_num, device=x.device)
        if self.share:
            for i in range(self.sig_num):
            ...
            for i in range(self.sig_num):
                ...
                loss_px[8*i:8*i+2] = rgb_loss(params, target)
                ...
                loss_px[8*i+2:8*i+4] = rgb_loss(params, target)
                ...
                loss_px[8*i+4:8*i+6] = rgb_loss(params, target)
                ...
                loss_px[8*i+6:8*i+8] = rgb_loss(params, target)
    ...
    def _decode_single(self, h:int, w:int, device:str, coder:MixDecoder):
        ...
        xt = th.zeros((1,1,h,w), device=device).float()
        # decode qz
...
class INS(nn.Module):
    ...
    def forward(self, x:th.Tensor):
        xsize = np.prod(x.shape)
        b = x.shape[0]
        bpds = th.zeros(8*self.ins_num, device=x.device)
        ...
        if self.share:
            ...
            for i in range(self.ins_num):
                ...
                prior = plane_context
                params = self.entropys[4*i](prior)[...,0::2,0::2]
                bpds[i*8:i*8+2] = rgb_loss(params, target.clone())
                ...
                params = self.entropys[4*i+1](prior)[...,1::2,1::2]
                bpds[i*8+2:i*8+4] = rgb_loss(params, target.clone())
                ...
                prior = th.cat([plane_context, space_context], dim=1)
                params = self.entropys[4*i+2](prior)[...,0::2,1::2]
                bpds[i*8+4:i*8+6] = rgb_loss(params, target.clone())
                ...
                prior = th.cat([plane_context, space_context], dim=1)
                params = self.entropys[4*i+3](prior)[...,1::2,0::2]
                bpds[i*8+6:i*8+8] = rgb_loss(params, target.clone())
        return bpds/xsize
...
class ARIB_BPS(nn.Module):
    ...
    def decompress_from_file(self, src, dst, device:str='cuda:0', precision=16):
        ...
        x_img_np = x_rec.squeeze(0).squeeze(0).cpu().numpy()
        Image.fromarray(x_img_np,mode='L').save(dst)
